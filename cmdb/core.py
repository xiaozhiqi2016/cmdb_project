...
class Asset(object):
    def __init__(self,request):
        ...
        self.mandatory_fields = ['sn','asset_id','asset_type'] #在客户端post过来的数据必须包含'sn','asset_id','asset_type'
        self.field_sets = {
            'asset':['manufactory'],
    ...
    def response_msg(self,msg_type,key,msg):
        '''如果数据中没有那些字段，reponse:{'error':['MandatoryCheckFailed':'he field [%s] is mandatory ...']}'''
    ...
    def save_new_asset_to_approval_zone(self):
        '''当有新资产汇报,需要管理员批准这条新资产添加到待批准数据库表中'''

        asset_sn = self.clean_data.get('sn')
        # get_or_craete会先get是否有这数据，没有就创建
        # https://docs.djangoproject.com/en/dev/ref/models/querysets/#get-or-create
        # 当您使用手动指定的主键时，get_or_create（）方法具有与create（）类似的错误行为。 如果需要创建对象，并且键已经存在于数据库中，则会引发IntegrityError。
        # 如果您使用MySQL，请确保使用READ COMMITTED隔离级别，而不是REPEATABLE READ（默认），否则您可能会看到get_or_create将引发IntegrityError但该对象不会出现在随后的get（）调用中的情况。
    ...
    def data_is_valid(self):
        ...
        if data:
            try:
                ...
                self.clean_data = data
                # self.repost中没有error信息,则要执行data_inject方法了
    ...
    def __is_new_asset(self):
        '''判断是不是新资产,判断self.asset_obj数据库中是否有self.clean_data['asset_type']新资产在资产总表中肯定是没有asset_type的,'''
    ...
    def data_inject(self):
        '''就把客户端传过来的数据保存到数据库中,data_is_valid()必须为true,'''
    ...
    def __update_asset_component(self,data_source,fk,update_fields,identify_field=None):
        '''
        对比数据库和客户端一个不变的值,来判断客户端的数据是否改变
        data_source: 客户端传过来的数据
        fk: 用于做Asset反向关联的字段,a = models.Assets.all(),a.nic_set.select_related()就能查询到nic表
                        ...
                        for source_data_item  in data_source:
                            ...
                            if key_field_data_from_source_data:
                                if key_field_data == key_field_data_from_source_data: #find the matched source data for this component,then should compare each field in this component to see if there's any changes since last update
                                   ...
                                   break #must break ast last ,then if the loop is finished , logic will goes for ..else part,then you will know that no source data is matched for by using this key_field_data, that means , this item is lacked from source data,
                                   # it makes sense when the hardware info got changed. e.g: one of the RAM is broken, sb takes it away,then this data will not be reported in reporting data
                        ...
                        for key,source_data_item  in data_source.items():
                            ...
                            if key_field_data_from_source_data:
                                if key_field_data == key_field_data_from_source_data: #find the matched source data for this component,then should compare each field in this component to see if there's any changes since last update
                                   ...
                                   break #must break ast last ,then if the loop is finished , logic will goes for ..else part,then you will know that no source data is matched for by using this key_field_data, that means ,
                                   # this item is lacked from source data, it makes sense when the hardware info got changed. e.g: one of the RAM is broken, sb takes it away,then this data will not be reported in reporting data
                            else: #key field data from source data cannot be none
                                self.response_msg('warning','AssetUpdateWarning',"Asset component [%s]'s key field [%s] is not provided in reporting data " % (fk,identify_field) )
    ...
    def __delete_components(self,all_components, delete_list , identify_field ):
        ...
        for obj in all_components:  # nic_obj1,nic_obj2....
        ...
        for field in fields_from_db:
            ...
            if val_from_data_source:
                ...
                if val_from_db == val_from_data_source:# this field haven't changed since last update
                    ...
                    db_field = model_obj._meta.get_field(field)  # 获取到某个字段对象
                    db_field.save_form_data(model_obj, val_from_data_source)  # 修改字段对象的值
                    model_obj.update_date = timezone.now()
                    model_obj.save()  # 改完值要保存
